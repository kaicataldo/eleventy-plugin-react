...
module.exports = function eleventyPluginReact(
    ...
    eleventyConfig.addExtension(ext, {
      ...
      compile(_str, inputPath) {
        return async (data) => {
          ...
          const artifacts = ReactDOM.renderToString(Component);

          babelHook.flushCache();

          if (typeof postProcess === "function") {
            return postProcess(artifacts, data);
          }

          return generateDefaultHtml(artifacts, data);
        };
      },
    });
  }
  ...
  babelHook.setup(ssrBabelConfig);
};
