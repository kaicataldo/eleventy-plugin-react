...
async function generateHTML({ bodyContent, helmet }) {
  let html = dedent`
    <!doctype html>
      <html ${removeHelmetDataAttribute(
        helmet.htmlAttributes.toString().trim()
      )}>
      <head>
        ${removeHelmetDataAttribute(helmet.title.toString().trim())}
        ${removeHelmetDataAttribute(helmet.meta.toString().trim())}
        ${removeHelmetDataAttribute(helmet.link.toString().trim())}
      </head>
      <body ${removeHelmetDataAttribute(
        helmet.bodyAttributes.toString().trim()
      )}>
        <div id="content">
          ${bodyContent}
        </div>
