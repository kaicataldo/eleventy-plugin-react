        ...
        return async (data) => {
          ...
          const processedHtml =
            typeof postProcess === "function"
              ...
              : generateDefaultHtml(html, data);

          return hydrationCache.hasComponents()
            ? transformHydrationMarkers(processedHtml)
            : processedHtml;
        };
      },
    });
    ...
    async function (content, outputPath) {
      ...
      if (
        ...
        const stats = await bundleClientAssets({
          ...
          extensions: exts.map((ext) => normalizeExtension(ext, true)),
          assetsPath,
        });
        const bundledClientAssets = Array.from(
