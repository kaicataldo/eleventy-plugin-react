      ...
      compile(_str, inputPath) {
        return async (data) => {
          const {
            ...
            pageData: pageDataFn,
          } = requireFromDir(inputPath, APP_ROOT);
          ...
          if (pageDataFn) {
            if (typeof pageDataFn !== "function") {
              throw new Error(
                "`pageData` export must be a function that returns an object or a Promise resolving to an object containing page-level data. Consider renaming this export if you aren't trying to set page-level data, as `eleventy-plugin-react` reserves this export for this purpose."
              );
            }

            const pageData = await Promise.resolve(pageDataFn(mergedData));
