...
function convertMarkers(html) {
  ...
  for (const el of startMarkerEls) {
    ...
    const hydrationData = el.textContent;
    newMarkerEl.setAttribute("data-hydration-props", hydrationData);
    el.remove();
  }
  ...
  for (const el of endMarkerEls) {
    ...
    const newMarkerComponentName = el.getAttribute(
      "data-hydration-component-name"
    );

    if (newMarkerComponentName !== componentName) {
      newMarkerEl.setAttribute("data-hydration-component-name", componentName);
    }
